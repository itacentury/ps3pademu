...
var indexEl = 0,
  nextEl = null,
  url = "http://ps3.aldostools.org";

//if(location.protocol!='http:'){location.href='http:'+window.location.href.substring(window.location.protocol.length);}

if (location.protocol == "https:")
  document.write(
    "<center><H1>WARNING: <font color=yellow>This site only works with HTTP protocol</font></CENTER><H1>"
  );
...
function refreshScreen() {
  ...
  req.ontimeout = function () {
    isoffline = true;
    /*showMenu();*/ window.open("/dummy.html", "portrait");
  };
  req.send();
}
...
(function () {
  var m = Math,
    ...
    iScroll = function (el, options) {
      ...
      // Normalize options
      that.options.useTransform = hasTransform
      ...
      that.scroller.style[vendor + "TransitionProperty"] = that.options
        .useTransform
      ...
      if (that.options.useTransform)
        that.scroller.style[vendor + "Transform"] =
          trnOpen + that.x + "px," + that.y + "px" + trnClose;
      else
  ...
  iScroll.prototype = {
    ...
    _scrollbar: function (dir) {
      ...
      if (dir == "h") {
        ...
        that.hScrollbarIndicatorSize = m.max(
          mround((that.hScrollbarSize * that.hScrollbarSize) / that.scrollerW),
          8
        );
        ...
        that.vScrollbarIndicatorSize = m.max(
          mround((that.vScrollbarSize * that.vScrollbarSize) / that.scrollerH),
          8
        );
    ...
    _scrollbarPos: function (dir, hidden) {
      ...
      that[dir + "ScrollbarIndicator"].style[vendor + "Transform"] =
        trnOpen + (dir == "h" ? pos + "px,0" : "0," + pos + "px") + trnClose;
    },
    ...
    _start: function (e) {
      ...
      if (that.options.momentum) {
        if (that.options.useTransform) {
          ...
          x =
            getComputedStyle(that.scroller, null).left.replace(/[^0-9-]/g, "") *
            1;
          y =
            getComputedStyle(that.scroller, null).top.replace(/[^0-9-]/g, "") *
            1;
        }
...
function clearCombo() {
  combo = 0;
  scombo = "";
  ComboMode.innerHTML = "Single Click";
}
...
function pad(param) {
  ...
  if (param == "SysInfo") {
    ...
    go("/pad.ps3?AnalogL_" + param + (gmode ? "_hold" : ""));
    clearCombo();
  } else if (isCtrlKey) {
    ...
    if (combo == 0 && gmode == 0) {
      switch (scombo) {
        ...
        case "l3_r2_cross_":
          go("/shutdown.ps3");
          break;
        case "select_r2_square_":
          go("/mount_ps3/net0");
          break;
        case "select_l2_square_":
          go("/mount_ps3/net1");
          break;
        case "r2_select_square_":
          go("/mount_ps3/net0");
          break;
        case "l2_select_square_":
          go("/mount_ps3/net1");
          break;
      }
    }
  ...
  if (isShiftKey || lstick) {
    if (param == "_up") btnAnalogL_up.focus();
    else if (param == "_down") btnAnalogL_down.focus();
    else if (param == "_left") btnAnalogL_left.focus();
    else if (param == "_right") btnAnalogL_right.focus();
    else if (param == "_up_left") btnAnalogL_up_left.focus();
    else if (param == "_up_right") btnAnalogL_up_right.focus();
    else if (param == "_down_left") btnAnalogL_down_left.focus();
    else if (param == "_down_right") btnAnalogL_down_right.focus();
  } else if (isCtrlKey || rstick) {
    if (param == "_up") btnAnalogR_up.focus();
    else if (param == "_down") btnAnalogR_down.focus();
    else if (param == "_left") btnAnalogR_left.focus();
    else if (param == "_right") btnAnalogR_right.focus();
    else if (param == "_up_left") btnAnalogR_up_left.focus();
    else if (param == "_up_right") btnAnalogR_up_right.focus();
    else if (param == "_down_left") btnAnalogR_down_left.focus();
    else if (param == "_down_right") btnAnalogR_down_right.focus();
  } else {
    if (param == "_up") btnUp.focus();
    else if (param == "_down") btnDown.focus();
    else if (param == "_left") btnLeft.focus();
    else if (param == "_right") btnRight.focus();
    else if (param == "_square") btnSquare.focus();
    else if (param == "_cross") btnCross.focus();
    else if (param == "_circle") btnCircle.focus();
    else if (param == "_triangle") btnTriangle.focus();
    else if (param == "_r1") btnR1.focus();
    else if (param == "_l1") btnL1.focus();
    else if (param == "_r2") btnR2.focus();
    else if (param == "_l2") btnL2.focus();
    else if (param == "_r3") btnR3.focus();
    else if (param == "_l3") btnL3.focus();
    else if (param == "_select") btnSelect.focus();
    else if (param == "_start") btnStart.focus();
    else if (param == "_psbtn") btnPS.focus();
    else if (param == "SysInfo") btnInfo.focus();
    else window.focus();
  }
...
function navigate(url) {
  ...
  else if (url.substring(0, 1) == "/") go("/browser.ps3" + url);
  else if (url == "psx-place") go("/browser.ps3?http://www.psx-place.com");
  else if (url == "forum") go("/browser.ps3?http://www.psx-place.com/forum");
  else if (url == "ps3devwiki") go("/browser.ps3?http://www.ps3devwikicom");
  else if (url == "brewology")
    go("/browser.ps3?http://store.brewology.com/whatsnew.php");
  else if (url == "imdb") go("/browser.ps3?http://imdb.com");
  else if (url == "gmail") go("/browser.ps3?http://gmail.com");
  else if (url == "psn") go("/browser.ps3*psn:");
  else if (url == "reg") go("/browser.ps3*regcam:reg?");
  else go("/browser.ps3?http://google.com/search?q=" + url);
}
...
function _go(param) {
  ...
  window.open(
    "http://" + ip + param,
    screen.height > screen.width ? "portrait" : "cpursx"
  );
...
function toggle_cpursx() {
  ...
  document.getElementById("ComboMode").style.display = !iframe_mode
    ? "none"
    : "";
}
...
function findNextTabStop(el) {
  ...
  var list = Array.prototype.filter.call(universe, function (item) {
    return (
      item.tabIndex >= "0" &&
      item.parentNode.parentNode.parentNode.id == "scroller"
    );
  });
...
function keyDownEvent(e) {
  ...
  if (keyCode == 27) {
    document.getElementById("wmframe").style.display = "none";
    document.getElementById("ComboMode").style.display = "";
    hideMenu();
  }

  if (document.getElementById("wrapper").style.display == "none") {
    if (keyCode == 38) {
      nextEl = findBackTabStop(nextEl);
      nextEl.focus();
      indexEl--;
      if (indexEl > 0) {
        return;
    ...
    if (keyCode == 33 || keyCode == 38) {
      if (myScroll.currPageY > 0) myScroll.currPageY--;
      myScroll.scrollToPage(0, myScroll.currPageY, 0);
    } //pgup
    if (keyCode == 34 || keyCode == 40) {
      if (myScroll.currPageY < 3) myScroll.currPageY++;
      myScroll.scrollToPage(0, myScroll.currPageY, 0);
    } //pgdn
  ...
  if (keyCode == 9) {
    buttons += isShiftKey ? "_start_select" : "_select";
  } // tab
  ...
  if (pressed[13] && isAltKey) {
    go("/xmb.ps3$exit;/browser.ps3$home");
    btnInfo.focus();
  } else if (pressed[13] && isCtrlKey) buttons += "_start";
  else if (pressed[13] && isShiftKey) buttons += "_start";
  else if (pressed[90] || pressed[13]) buttons += "_cross"; // z
  if (pressed[88] || pressed[27] || pressed[8]) buttons += "_circle"; // x
  if (pressed[65]) buttons += "_square"; // a
  if (pressed[83]) buttons += "_triangle"; //s

  if (pressed[81] || pressed[36])
    buttons += isShiftKey || isCtrlKey ? "r1+r2+l1+l2+l3+down" : "_l1"; // q
  ...
  if (pressed[221] || pressed[220] || pressed[190] || pressed[191])
    buttons += "_start"; // ] . / \
  ...
  if (pause && buttons == "_select") buttons = "_start";
  else pause = false; // disable hack with any other key

  if (pressed[72]) {
    go("/browser.ps3$home" + (isCtrlKey ? "" : "*"));
    delete pressed[72];
    btnInfo.focus(); // h
  } else if (pressed[73]) {
    go("/popup.ps3");
    ...
    go("/play.ps3?col=network&seg=seg_premo");
    btnInfo.focus();
  } // F12
  else if (pressed[36] || pressed[103]) {
    //home
    if (isCtrlKey && isShiftKey) {
      go("/browser.ps3$focus_category music");
    } else if (isCtrlKey) {
      go("/browser.ps3$focus_category sysconf");
    } else if (isShiftKey) {
      go("/browser.ps3$focus_category user");
    } else {
      go("/browser.ps3$scroll_list -9999");
    }
    btnInfo.focus();
  } else if (pressed[35] || pressed[97]) {
    //end
    if (isCtrlKey && isShiftKey) {
      go("/browser.ps3$focus_category video");
    } else if (isCtrlKey) {
      go("/browser.ps3$focus_category network");
    } else if (isShiftKey) {
      go("/browser.ps3$focus_category friend");
    } else {
      go("/browser.ps3$scroll_list 9999");
    }
    btnInfo.focus();
  } else if (pressed[12] || pressed[101]) {
    //5
    if (isCtrlKey && isShiftKey) {
      go("/browser.ps3$home*");
    } else if (isCtrlKey) {
      go(
        "/browser.ps3$focus_category game;/browser.ps3$focus_segment_index seg_device"
      );
    } else if (isShiftKey) {
      go(
        "/browser.ps3$focus_category game;/browser.ps3$focus_segment_index seg_gamedebug"
      );
    } else {
      pad("cross");
    }
    btnInfo.focus();
  } else if (pressed[33] || pressed[105]) {
    //page up
    go("/browser.ps3$scroll_list -5");
    btnInfo.focus();
  } else if (pressed[34] || pressed[99]) {
    //page down
    go("/browser.ps3$scroll_list 5");
    btnInfo.focus();
  } else if (isCtrlKey && pressed[77]) {
    //ctrl+m
    go("/browser.ps3$music");
    btnInfo.focus();
  } else pad(buttons);
...
if (ip.length > 0 && ip != "127.0.0.1" && ip != "localhost") {
  setCookie("ip", ip, 10000);
}
